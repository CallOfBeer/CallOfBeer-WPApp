    ...
    public sealed partial class RegisterPage : Page
    {
        ...
        private async void BTN_go_Tapped(object sender, TappedRoutedEventArgs e)
        {
            if (string.IsNullOrEmpty(TXT_mail.Text)
                || string.IsNullOrEmpty(TXT_login.Text)
                || string.IsNullOrEmpty(TXT_pass.Password)
                || string.IsNullOrEmpty(TXT_pass_conf.Password))
            {
                MessageDialog messageBox = new MessageDialog("Vous devez renseignez tous les champs pour continuer.");
                await messageBox.ShowAsync();
            }
            else
            {
                if (TXT_pass.Password != TXT_pass_conf.Password)
                {
                    MessageDialog messageBox = new MessageDialog("Les deux mots de passe ne correspondent pas.");
                    await messageBox.ShowAsync();
                }
                else
                {
                    ...
                    Frame.Navigate(typeof(MainPage), loginModel);
                }
            }
        }
